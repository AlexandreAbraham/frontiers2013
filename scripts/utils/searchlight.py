...
class SearchLight(BaseEstimator):
    ...
    def fit(self, niimgs, y):
        ...
        if self.process_mask_img is None:
            ...
            if isinstance(process_mask_img, basestring):
                process_mask_img = nibabel.load(process_mask_img)
            process_mask = process_mask_img.get_data().astype(bool)
            process_mask_affine = process_mask_img.get_affine()
        ...
        X = masking.apply_mask(niimgs,
                nibabel.Nifti1Image(mask.astype(np.int8), mask_affine))
