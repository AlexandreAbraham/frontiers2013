...
of spatial structure. However, we have eliminated many uninformative
features: since not all voxels in fMRI data hold
relevant information, outer-brain voxels are of no
use and they may bring spurious noise and scanner artifacts.
Thus, keeping only voxels in a brain mask improves the ratio of signtificant
features. Such masks are often given along with the datasets or can be computed
...
displays, as an overlaying map, the voxels with highest value.
...
All these objects are implemented as transformers (see
 section~\ref{scikitlearn}) and are easily invertible (see code below).
...
seen by the subject and the associated brain activation map.
...
As we expect brain voxels to be sparsely distributed in the brain, we use the
