...
In the following examples, all figures are made inside python scripts using
matplotlib to display an axial slice. Most of them are composed of two layers: a
gray background and a colorful activation map. Overlaying them is done using a
\texttt{numpy.ma.masked\_array}. Note that, to be displayed
properly, data must be transposed and the origin of plot must be changed. 
...
Several tricks exist to enhance the overall aspect of the plot (like removing
the default large white margins). One can look at image generation in our
example scripts\footnote{http://www.github.com/AlexandreAbraham/frontiers2013/}
to learn more about image generation using matplotlib. Nipy provides a
\texttt{plot\_map} function that is tuned to display activation maps (a
background is even provided).

The simplest way to visualize MRI data in an interactive way is to dump data in
a file and use an external viewer like FSLview or MRIcron. % or use Pynax !
3-dimensional display can be achieved in Python using Mayavi and from file using
MRIcrogl.
...
a relation between brain voxels and observed pixels. 
...
figure~\ref{fig:miyawaki}. This is particularly important for Logistic
Regression which performance can dramatically decrease outside of the optimal
value ($C=0.05$).
...
\begin{figure}[htbp]
    ...
    \hline\\[-.9em]
    \hline\\[-.7em]
        Logistic Regression ($\ell_1$) & 0.5011 & 0.5010 & 0.5793 & 0.6392 &
                              \textbf{0.7080} & 0.7030 \\[.1em]
        $\ell_1$ SVC        & 0.5099 & 0.5583 & 0.6936 &
                              \textbf{0.7103} & 0.6946 & 0.6843 \\[.1em]
        $\ell_2$ SVC        & 0.6771 & \textbf{0.6793} &
                              0.6722 & 0.6683 & 0.6584 & 0.6553
    \end{tabular}
    \end{center}
...
Figure~\ref{fig:miyawaki}. shows encoding . We can
...
visual stimuli. Moreover, the two different estimators used in decoding
give similar results in terms of prediction accuracy for the different
pixels.
...
\begin{figure}[hbtp]
  ...
  \caption{
      ...
      \textbf{Top: Decoding.} Reconstruction accuracy per pixel 
      (\textit{a.} Logistic
      regression, \textit{c.} SVC). Classifier weights for the pixel
      highlighted (\textit{b.} Logistic regression, \textit{d.} SVC). 
      \textbf{Bottom: Encoding.} Left: reconstruction accuracy depending on 
	pixel position in the stimulus. Right: receptive fields corresponding to
       voxels with highest scores and its neighbors.
      --- Note that the pixels and voxels highlighted are the same in 
