...
and computer scientists working in neuroscience,
...
on the various test sets can be to give give a quantitative score
...
template like the MNI one if necessary. Reference softwares for these tasks are
...
\bertrand{I dont understand the last sentence.}
...
display an axial slice overlaid with an activation map. Only
the voxels with highest values are displayed.
\bertrand{With a value of 6000, this is probably not an activation map. What is it ?}
...
Two strategies exist to reduce the number of features.
A first strategy is based on prior neuroscientific
knowledge. Here one could restrict the mask to occipital areas, where the visual
cortex is actually located. A second approach is a data-driven. Features
...
As one can indeed expect the number of predictive voxels to be limited, we use 
a simple Logistic Regression (LR) penalized with the $\ell_1$ norm
known to promote sparsity. The SVM classifier used above and the LR
differ by their loss functions: SVM uses a hinge loss while the LR
...
Table~\ref{fig:miyawaki_cv} compares the described approaches, along with an
...
regularized method does better than $\ell_2$ regularized method. SVC and
Logistic Regression achieve the same score, which is not surprising as they 
are implement similar models.
...
regression --the Lasso-- to find the receptive fields. Here we use the
\emph{LassoLarsCV} estimator that relies on the Lars algorithm and
...
In decoding settings, Figures \textit{a} and \textit{c} show the classifiers weights as brain
...
see that receptive fields of neighboring voxels are neighbouring
