...
\end{itemize}
\michael{Do we need to add that inversion is not always exact? E.g. by saying "If the transformation can be inverted in a certain sense''}
...
\michael{is K-Fold now always random?}
...
\alex{I think it means that if you have a diagonal affine with 3 3 2, you can
    change the scaling in third direction to get something isotropic for
example. I think we can remove this sentence.}
...
includes:
\michael{Poor resolution? With respect to what?}
\begin{itemize}
...
\michael{I understand there may be reasons for choosing the colormap employed. However, in the encoding case, having a pixel value that is really close to 0, i.e. really close to not being selected if the penalty were slightly different, scream at me in bright turquoise WILL distort my view on the receptive field: They look awfully spread out, while they are actually not, if you manage to focus on the purple and magenta pixels}
}
\label{fig:miyawaki}
\end{figure}
...
\michael{K-means or K-Means? Both are used in this paragraph}
