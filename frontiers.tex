...
Matplotlib is a plotting library that is part of the scientific python stack.
It offers a Matlab-like experience and allows to display plots, images or even
3D plots in a graphical user interface. All figures in this paper have been
generated using this library.

Note that a more convenient interface for matplotlib called pylab allows a
procedural (ie more pythonish) use of matplotlib.

\fabian{while I agree in practice with you, pylab was built as a  is a matlab-like API, while matplotlib
was designed to be object oriented and (in theory) more Pythonic. I think its
better to not go into the discussion of which is more pythonic}
...
All objects within {\em scikit-learn} are organized around three key concepts: 


API consisting of
three complementary interfaces: an {\em estimator} interface 

Interoperation with third-party code is OK as long as the code implements the
needed methods, which 

Scikit-learn not only provides a variety of statistical learning algorithms,
but everything needed to preprocess data and validate the results. For the sake
of usability, all the scikit-learn functionalities have been divided in several
simple concepts.
