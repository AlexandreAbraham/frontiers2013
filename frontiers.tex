...
\subsubsection{From 4-dimensional images to 2-dimensional array}
...
In order to chose the classifier adapted to our problem, we have to make some
prior assumptions.

Also, from neuroscientific knowledge,
we expect that the value of a single pixel will be related
to only few voxels.

From these observations, we look for classifiers penalized sparsity inducing norm,
such as $\ell_1$. We chose different models using different losses: logistic
regression (used in original work) and Linear SVC.
...
Using logistic regression makes sense considering that it can be extended to
multiclass for multiscale classification.

Here we use a simpler version of this approach, a $\ell_1$ logistic
regression, and compare it to the previously seen SVC.
...
learning. 
To extract functional network, we use methods that group together 
similar voxels by comparing their time
series. In neuroimaging, the most popular method is ICA and
is the subject of our first example. We will then show results obtained with
clustering methods.
...
A clustering method aggregates samples into groups (called clusters)
based on a similarity measure. Some clustering approaches can make use of
...
\begin{itemize}
    \item{\bf Ward's clustering} uses a bottom-up hierarchical approach. Features are
        progressively aggregated into clusters.
...
% XXX: recall the necessary transpose
