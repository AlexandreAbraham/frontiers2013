...
% Scikit-learn world
% ------------------
% - X: unmasked func_data (scikit-learn world)
% - y: labels
...
important to resample all files on the same affine.
...
reduction (using PCA) followed by a concatenation of timeseries.
...
others evoked methods (CanICA, MELODIC).
...
\begin{itemize}
    ...
    \item{\bf k-means} is a centroid based approach.
    \item{\bf spectral clustering} is a graph based approach. It separates
        features into cliques by maximizing intra-group connectivity and
        minimizing inter-group connectivity.
\end{itemize}

For eadch algorithm, one must specify the number of clusters. This parameter is
...
We use a PCA to reduce the dimensionality of the problem. Some clustering
algorithms (like k-means) give better results if data is smoothed.
